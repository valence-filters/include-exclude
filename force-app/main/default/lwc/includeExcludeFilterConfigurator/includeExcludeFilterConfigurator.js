...
export default class IncludeExcludeFilterConfigurator extends ValenceUIConfigurator {
	conditionOptions = [
		...
		{ label: "MUST NOT", value: "MUST NOT" },
		{ label: "MUST START WITH", value: "MUST START WITH" },
	];
	...
	computeValid() {
		...
		return (
			Boolean(this.configuration.condition) &&
			this.configuration.values.length > 0 &&
			Boolean(this.configuration.ignoreReason)
		);
	}
}